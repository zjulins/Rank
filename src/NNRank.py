...
class NNRank(object):
    ...
    def __init__(self, cfg, lang = 'ENG', model_file = None, train_file = None,test_file = None):
...
#                                att_word_emb ], axis = 2)
    ...
    def L2(self):
        return (self.matrix_emb_h1*self.matrix_emb_h1).sum()+(self.matrix_h1_h2*self.matrix_h1_h2).sum()+(self.matrix_h2_out*self.matrix_h2_out).sum()+(self.matrix_emb_h1_NIL*self.matrix_emb_h1_NIL).sum()+(self.matrix_h1_h2_NIL*self.matrix_h1_h2_NIL ).sum()+(self.matrix_h2_out_NIL*self.matrix_h2_out_NIL).sum()
    ...
    def train_functions(self, trainMod):
        ...
        cost     = TT.nnet.categorical_crossentropy(probs, TT.cast(lab[:,:,0], dtype = 'float32')).mean()+0.02/lab.shape[0]*self.L2()
    ...
    def test_functions(self, trainMod):
        ...
        out     = TT.concatenate([out0, out1], axis = 1).reshape((self.batch_size, lab.shape[1]))
        out_max = out.max(axis = 1) # (bSize)
        out_exp = TT.exp(out) * TT.cast(lab[:,:,1], dtype = 'float32') + TT.cast(1e-8, dtype = 'float32')
        out_sum = out_exp.sum(axis = 1)
        probs   = out_exp / out_sum.dimshuffle(0,'x')
        #probs = dbg_hook_shape('probs', probs)
        ft = theano.function([fea,lab], out_exp, allow_input_downcast = True,on_unused_input = 'ignore')
    ...
    def train(self):
        ...
        while(iter < self.epoc):
            ...
            while(trainDataProvider.isEnd() == 0):
                ...
                tmp = self.processOneBatch(aBatch, fea, lab, lr, fcost , fupdata) #return cost
                if(tmp >= 0):
                    cost = cost + tmp
    ...
    def testOneBatch(self, aBatch, fea, lab, ftest,ft):
        rlt=0
        num=0
        if(aBatch is None):
